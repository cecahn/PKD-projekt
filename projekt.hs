...
PRE: True
RETURNS: First the instructions for the game and the list of letters the player will be able to use this round. Then a few different
strings depending on the players input in the program. 
EXAMPLES : 

Welcome to Alphapet the computergame 
...
"jwdmtrthwvnkbnuqqxczbhizmuksrt"
...
reads the file english3 and checks the approved words against it. Stores and presents all the information from the previous rounds
and starts a new round if the player chooses to do so.  
PRE = 
RETURNS =
EXAMPLES =     
